    ...
    <div v-for="(item, index) in resumeList" :key="index" :hidden="!hasContent">
      <el-card shadow="hover" class="receiveBox">
        <div class="flex">
          ...
          <el-button  class="clickbtn" @click="clickUnprocessedResume(item)">返回待处理</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getResumeList() {
        this.$axios.post("/jobseekerJobRelation/list",{userId:this.loginInfo.userInfo.id,status:3}).then(res=>{
          if (res.code == 200){
