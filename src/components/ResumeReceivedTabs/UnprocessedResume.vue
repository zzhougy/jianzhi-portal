...
<!--          <span class="pipei">简历匹配度</span>-->
          ...
          <el-button @click="clickToBeCommunicated(item)" class="clickbtn">待沟通</el-button>
          <el-button  class="clickbtn" @click="clickImproperResume(item)">不合适</el-button>
          <el-button class="clickbtn" @click="goToAsk(item)">在线询问</el-button>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getResumeList() {
        ...
        this.$axios.post("/jobseekerJobRelation/list",{userId:this.loginInfo.userInfo.id,status:1}).then(res=>{
          if (res.code == 200){
          ...
          console.log('ssssssss' , res.data.records)
        })
      },
      getTableList(id) {
      ...
      //点击了待沟通
      clickToBeCommunicated(val){
        this.editResumeStatus(val.id,2)
      },
      ...
      goToAsk(val){
        ...
        location.href=`${process.env.VUE_APP_BASE_API_IM}/sys-check?t=${sessionStorage.getItem('JWT_TOKEN')}&to=${val.jobSeeker.username}&return_url=${return_url}`
      }
    }
  }
</script>
