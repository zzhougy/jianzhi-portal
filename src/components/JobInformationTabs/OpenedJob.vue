<template>
  <div class="wrapper">
    ...
    <el-card  v-for="(item, key) in joblist" :key="key" class="mycard"  :hidden="!hasContent" >
      ...
      <el-button class="delbtn" @click.native="checkDetail(item.id)">查看详情</el-button>
      <el-button class="delbtn" @click=closeJob(item.id)>关闭岗位</el-button>
      <el-button class="delbtn" @click=editJob(item.id)>编辑岗位</el-button>
    </el-card>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    closeJob (id) {
      ...
      this.$confirm("是否继续关闭?", "提示", {
        ...
        type: "warning",
          ...
          .then(() => {
            console.log("确认关闭");
            this.$axios.post("/jobInfo/edit",{id:id,status:2}).then(res=>{
              if(res.code==200){
                this.queryJobList()
              }
            })
    ...
    queryJobList(){
      this.$axios.post("/jobInfo/list",{userId:this.loginInfo.userInfo.id,status:1}).then(res=>{
        if (res.code == 200){
          console.log('res' , res)
    ...
    editJob(val){
      console.log('val' , val)
      this.$router.push({path:'/jobInformationView',query:{activeName:'third',jobId:val}});
      console.log('5555'  )
    }
  },
}
