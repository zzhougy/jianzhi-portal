...
<!--        <li :class="{now: fourNow.includes(1)}" @click="changeFour(1)"><a href="javascript:">餐饮</a></li>-->
<!--        <li :class="{now: fourNow.includes(2)}" @click="changeFour(2)"><a href="javascript:">景点</a></li>-->
<!--        <li :class="{now: fourNow.includes(3)}" @click="changeFour(3)"><a href="javascript:">其它</a></li>-->
<!--      </ul>-->
<!--    </div>-->
    <div class="list_content">
      <ul class="content_ul clearfix">
        <li :class="{now: oneNow.includes(0)}" @click="changeOne(0)"><a href="javascript:" >全部</a></li>
      <li :class="{now: oneNow.includes(item.id)}" v-for="(item, index) in unitLabelList" :key="index" @click="changeOne(item.id)"><a href="javascript:">{{item.name}}</a></li>
      </ul>
    </div>
  </div>
</template>
  ...
  data () {
   ...
   methods: {
    changeOne (val) {
     ...
     // },
     queryUnitLabelList(){
       this.$axios.post("/recruitUnitLabel/list",{}).then(res=>{
           if(res.code==200){
             this.unitLabelList = res.data.records
             console.log('33'  )
           }
       })
     }
